...
class ZMXParser(BaseLogger):
    ...
    def read_surf_block(self, surfblk):
        ...
        self.add_keyword_to_dict(                                     # equivalent KDP-2
            blocklines, "SURF", paramsdict,                           # THE FOLLOWING DATA REFERS TO SURFACE
            self.extract_first_arg_for_first_keyword_from_block,
            int)
        self.add_keyword_to_dict(
            blocklines, "STOP", paramsdict,                           # ASTOP
            self.is_keyword_in_block)
        self.add_keyword_to_dict(
            blocklines, "TYPE", paramsdict,                           # non-existent --> always standard if not special
            self.extract_first_arg_for_first_keyword_from_block,
            str)
        self.add_keyword_to_dict(
            blocklines, "PARM", zmxparams,                            # not sure
            self.extract_args_for_multiple_keywords_from_block,
            int, float)
        self.add_keyword_to_dict(                                     # thickness ... th
            blocklines, "DISZ", paramsdict,
            self.extract_first_arg_for_first_keyword_from_block,
            float)
        self.add_keyword_to_dict(                                     # diameter ... clap
            blocklines, "DIAM", paramsdict,
            self.extract_first_arg_for_first_keyword_from_block,
            float)
        self.add_keyword_to_dict(
            blocklines, "COMM", paramsdict,                           # C ... comment
            self.extract_string_for_first_keyword_from_block)
        self.add_keyword_to_dict(
            blocklines, "CURV", paramsdict,                           # CV ... curvature
            self.extract_first_arg_for_first_keyword_from_block,
            float)
        self.add_keyword_to_dict(
            blocklines, "CONI", paramsdict,                           # CC ... conic
            self.extract_first_arg_for_first_keyword_from_block,
            float)

        self.add_keyword_to_dict(
            blocklines, "GLAS", zmxglasdict,                          # not AIR ...
        ...
        self.add_keyword_to_dict(
            ...
            float, float)
        self.add_keyword_to_dict(
            blocklines, "CLAP", paramsdict,                       # clear aperature ... CLAP ?
            self.extract_args_for_first_keyword_from_block,
            float, float)
        self.add_keyword_to_dict(
    ...
    def read_name_and_notes(self):
        ...
        self.add_keyword_to_dict(
            self.__textlines, "NAME", zmxnamenotes,                       #  LI
            self.extract_string_for_first_keyword_from_block)

        return (zmxnamenotes["NAME"], zmxnamenotes["NOTE"])

    def read_field(self):                                                 # SAY, ...
        """
        ...
        self.add_keyword_to_dict(
            self.__textlines, "FTYP", zmxftyp,                           # not available
            self.extract_args_for_first_keyword_from_block,
